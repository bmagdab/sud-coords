...
%This is illustrated by sentences (\ref{ex:UD father}) and (\ref{ex:UD ballet}), which are shortened versions of two sentences from the GUM corpus (\texttt{GUM\_whow\_ballet-14} and \texttt{GUM\_fiction\_beast-36} respectively) annotated in UD (above the sentence) and in SUD (below the sentence). Some dependencies are not labelled and \texttt{punct} dependencies have been omitted for better clarity of the structures.
...
\begin{exe}
	...
	\depedge[show label, edge height=2cm]{6}{10}{conj} 
	\depedge{12}{11}{case} 
